...
let parse (input: string) =
    ...
    |> Seq.toList
    |> List.map (fun row -> row.Split [|'\t'|] |> Seq.toList |> List.map int)

let part1 (spreadsheet: int list list) : int =
    spreadsheet
    |> List.map (fun row -> List.max row - List.min row)
    |> Seq.sum

let part2 (spreadsheet: int list list) : int =
    let findEvenlyDivisibleResult (row: int list) : int =
        // Assumes that only one matching pair exists in a row.
        [for x in row do
         for y in row do
         if x <> y && x % y = 0 then yield x / y]
        |> List.head
    spreadsheet
    |> List.map findEvenlyDivisibleResult
    |> Seq.sum

printfn "%d" (myInput |> parse |> part1)
printfn "%d" (myInput |> parse |> part2)
